...
builtin_cd(struct command *cmd, struct builtin_redir const *redir_list)
{
   ...
   */
  // If the parsed command is not empty, set the target directory to the argument to cd
  // Defaults to home if command is empty
  if (cmd->words[1]!= NULL){
    target_dir = cmd->words[1];
  }
  setenv("PWD", target_dir, 1); // Set PWD environment variable
  if (chdir(target_dir)< 0) return -1;  // Change directory
...
builtin_exit(struct command *cmd, struct builtin_redir const *redir_list)
{
  /* TODO: Set params.status to the appropriate value before exiting */
    ...
    //  If there is an argument to exit, parse it into an integer value
    if (cmd->word_count == 2) {
        errno = 0;
        parsed_val = strtol(cmd->words[1], &endptr, 10);
        exit_val = (int)parsed_val;
        
    }

    // Otherwise the argument is invalid
    else if (cmd->word_count >= 3 || isalpha(cmd->words[1])){
      return -1;
    } 
    // Otherwise set exit_val to the last exit status
    else {
       
        exit_val = params.status;
    }

    params.status = exit_val; // Update the exit status
    bigshell_exit();
    return 0;
}
...
builtin_export(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  for (size_t i = 1; i < cmd->word_count; ++i) {
    ...
    char *v = strchr(word, '=');  // Returns the first occurence of '='
    if (v) {  // Once it encounters the assignment operator
      *v = '\0';
      if (vars_set(word, v + 1) < 0) return -1; // Set 
