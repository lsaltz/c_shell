...
{
  // Loops through the list of commands and expands them
  size_t i;
  for (i = 0; i < cmd->word_count; ++i) {
    expand(&cmd->words[i]);
  }
  for (i = 0; i < cmd->assignment_count; ++i) {
    expand(&cmd->assignments[i]->value);
  }
  for (i = 0; i < cmd->io_redir_count; ++i) {
...
do_variable_assignment(struct command const *cmd, int export_all)
{
  for (size_t i = 0; i < cmd->assignment_count; ++i) {
    ...
    vars_set(a->name, a->value);  // Sets the variable
    if (export_all != 0){ // Exports it if the export flag is set
      vars_export(a->name);
    }
    else{;}
    /* TODO Assign */
...
get_io_flags(enum io_operator io_op)
{
   ...
   * POSIX 1.2008
   */
  // Refer to https://www.gnu.org/software/libc/manual/html_node/File-Status-Flags.html
  /*
   * O_RDONLY: Read mode
   * O_WRONLY: Write mode
   * O_CREAT: Create
   * O_EXCL: Setting with O_CREAT guarantees open will fail if file exists
...
move_fd(int src, int dst)
{
  ...
  if (dup2(src, dst) < 0){
    dst = tmp1;
    src = tmp2;
  } // Move src to dst, if it fails then restore previous values
  close(src);  // Closes src
  ...
  for (size_t i = 0; i < cmd->io_redir_count; ++i) {
    ...
    if (r->io_op == OP_GREATAND || r->io_op == OP_LESSAND) {
      ...
      if (strcmp(r->filename, "-") == 0) {
         ...
         */
         close(r->io_number); // Close n = r->io_number
      } else {
        /* The filename is interpreted as a file descriptor number to
         * redirect to. For example, 2>&1 duplicates file descriptor 1
        ...
        if (*(r->filename) && !*end /* <--- this is part of the strtol idiom */
          ...
          if (dup2(src, r->io_number) < 0) goto err;  // Duplicate without closing
        } else {
    ...
    file_open:;
      int flags = get_io_flags(r->io_op);
      gprintf("attempting to open file %s with flags %d", r->filename, flags);
      int fd = open(r->filename, flags, 0777);  // Open the file
      if (fd < 0) goto err; // If it fails, err
      if(move_fd(fd, r->io_number) < 0) goto err; // Move src (file descripter) to dst (redirection target)
...
run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
    ...
    int const upstream_pipefd = pipeline_data.pipe_fd;  // Initialize to READ side
    ...
    int pipe_fds[2] = {-1, -1}; // Initialize to -1 for no pipe yet
    if (is_pl) {
      if (pipe(pipe_fds) < 0) goto err;
    }
    ...
    int const did_fork = (!is_builtin || !is_fg); // Set to 0 if it is a builtin or foreground command, otherwise set to 1
    if (did_fork) {
      child_pid = fork(); // Create a new process. Returns 0 to child and process ID to parent if newly created, errors out if fork fails.
      if (child_pid < 0) goto err;  // If the fork fails, error out
      /* [TODO] fork */
      ...
      // setpgid either joins or creates a new process group
    ...
    if (child_pid == 0) {
      if (is_builtin) {
        ...
        // STDIN_FILENO and STDOUT_FILENO are fds to be used for reading and writing
        if (has_upstream_pipe) move_fd(upstream_pipefd, STDIN_FILENO);  
        if (has_downstream_pipe) move_fd(downstream_pipefd, STDOUT_FILENO);
         ...
         */
        execvp(cmd->words[0], cmd->words);  // Execute command. Returns if it fails.
        err(127, 0); /* Exec failure -- why might this happen? */
        // https://stackoverflow.com/questions/1763156/127-return-code-from
        // Fails if the command is not found, thinks you're speaking gibberish
