...
is_valid_varname(char const *name)
{
  ...
  assert(name);
  /*Checks all characters for:
   * !alnum - Special characters
   * strcmp - Underscore checking
   * isdigit - First character for number
   * Can be alphanumeric or contain underscores
   * Cannot begin with a number
   */ 
  if (isdigit(name[0])) return 0;
  for (int i = 1; i < strlen(name); i++){;
    if (!isalnum(name[i]) && strcmp(&name[i], "_")!=0){
       return 0;
    }
  }
    /* TODO: Implement me.
...
vars_is_valid_varname(char const *name)
{
   ...
   * is_valid_varname() function. */
   // Checks if variable name is there
   if (!name) {
        return 0;
    }
   return is_valid_varname(name); // tail call
  //errno = ENOSYS;
}
