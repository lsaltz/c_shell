 ...
 *   - SIGTSTP  ^Z
 *   - SIGINT ^C
 *   - SIGTTOU Reading or Writing from background
 *
...
signal_init(void)
{
   ...
   *
   * */
  // Change action to ignore, saves previous action
...
signal_enable_interrupt(int sig)
{
  ...
  // Change action without storing the old action
...
signal_restore(void)
{
   ...
   *
   * */
   // Changes all signals back to their old actions
   if (sigaction(SIGINT, &old_sigint, NULL) < 0 || sigaction(SIGTTOU, &old_sigttou, NULL) < 0 || sigaction(SIGTSTP, &old_sigtstp, NULL) < 0) {
    return -1;
  }
  //errno = ENOSYS; /* not implemented */
  return 0;
