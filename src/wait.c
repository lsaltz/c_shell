...
// https://unix.stackexchange.com/questions/520127/cant-add-a-child-process-in-the-foreground-process-group
// https://www.man7.org/linux/man-pages/man3/tcgetpgrp.3.html
...
wait_on_fg_pgid(pid_t const pgid)
{
   ...
   * XXX review kill(2)
   */
  kill(-pgid, SIGCONT); // Signal is sent to every process in the group
  if (is_interactive) {
    /* TODO make 'pgid' the foreground process group
     * XXX review tcsetpgrp(3) */
     tcsetpgrp(STDIN_FILENO, pgid); //  STDIN_FILENO is the shell terminal
     // This sets the foreground ID to pgid
  }
  ...
  for (;;) {
    ...
    if (res < 0) {
      ...
      if (errno == ECHILD) {
        ...
        if (WIFEXITED(status)) {
          ...
          params.status = WEXITSTATUS(status);  // Get the correct exit status.
        } else if (WIFSIGNALED(status)) {
          /* TODO set params.status to the correct value */
          params.status = WTERMSIG(status) + 128;  // 128 is general convention
        }
    ...
    if (WIFSTOPPED(status)) {
      params.status = WSTOPSIG(status) + 128; // Get the stop signal status
      fprintf(stderr, "[%jd] Stopped\n", (intmax_t)jid);
      kill(-pgid, SIGCONT); // Continue the process group.
      goto out;
    }
...
out:
  ...
  if (is_interactive) {
     ...
     */
     ...
     // This gets the calling process group ID (bigshell) and sets it to the terminal 
  }
  return retval;
}
...
wait_on_bg_jobs()
{
  ...
  for (size_t i = 0; i < job_count; ++i) {
    ...
    for (;;) {
       ...
       */
      int status;
      pid_t pid = waitpid(pgid, &status, WUNTRACED | WNOHANG);  // Return immediately and report stopped processes.
      if (pid == 0) {
